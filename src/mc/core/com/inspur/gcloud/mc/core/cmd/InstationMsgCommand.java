...
public class InstationMsgCommand {
	...
	public String forwardInstationMsgList(@PathVariable(value = "boxType") String boxType){
		// 根据消息箱类型跳转至对应页面
		if(McSystemConstants.INSTATIONMSG_IN_BOX.equals(boxType)){
			// 收件箱
			return "mc/instationmessage/inbox/msg_inbox_query";
		}else if(McSystemConstants.INSTATIONMSG_OUT_BOX.equals(boxType)){
			// 发件箱
			return "mc/instationmessage/outbox/msg_outbox_query";
		}else if(McSystemConstants.INSTATIONMSG_DRAFT_BOX.equals(boxType)){
			// 草稿箱
			return "mc/instationmessage/draftbox/msg_draftbox_query";
		}else if(McSystemConstants.INSTATIONMSG_SCRAP_BOX.equals(boxType)){
			// 废件箱
	...
	public Map<String, Object> getInMessageList(@RequestBody Map<String, String> parameters) {
		...
		List<Map<String, Object>> mapList = instationMsgService.findInstationMsgList(parameters);
    ...
    public Map<String, Object> getByParams(@RequestBody Map<String, String> parameters) {
        ...
        List<Map<String, Object>> mapList = instationMsgService.getByParams(parameters);
    ...
    public MessageView replayMessage(@PathVariable(value = "messageId") String messageId,
    	...
    	if(messageId != null && !"".equals(messageId)){
    		...
    		Envelope envelope = instationMsgService.findEnvelopeByMessageIdAndLoginId(map);
    		Message message = instationMsgService.findMessageById(messageId);
    		// 构建回复页面视图对象
    		...
    		if(envelope.getReceiveState() == null || "0".equals(envelope.getReceiveState())){
        		...
        		instationMsgService.updateEnvelope(envelope);
    		}
    	}
    	return messageView;
    }
    ...
    public MessageView forwardMessage(@PathVariable(value = "messageId") String messageId,
    	...
    	if(messageId != null && !"".equals(messageId)){
    		...
    		//更新收件箱envelope状态
    		if(boxType.equals(McSystemConstants.INSTATIONMSG_IN_BOX)){
    			envelope.setReadTime(new Date());
    			envelope.setReceiveState("1");
    			instationMsgService.updateEnvelope(envelope);
    		} else {
    ...
    public MessageView editMessage(@PathVariable(value = "messageId") String messageId,
    	...
    	if(messageId != null && !"".equals(messageId)){
    		// messageId不为空，获取组装视图对象的Envelope对象和Message对象
    		List<Envelope> envelopeList = instationMsgService.findEnvelopeListByMessageId(messageId);
    		Message message = instationMsgService.findMessageById(messageId);
    		// 构建转发页面视图对象
    ...
    public MessageView viewMessage(@PathVariable("messageId") String messageId, 
    	...
    	parameterMap.put("boxType", boxType);
    	// 判断messageId是否为空
    	if(messageId != null && !"".equals(messageId)){
    		// messageId不为空，获取组装视图对象的Envelope对象和Message对象
    		List<Envelope> envelopeList = instationMsgService.findEnvelopeListByMessageIdAndLoginId(parameterMap);
    		Message message = instationMsgService.findMessageById(messageId);
    		// 构建页面展示消息对象
    		...
    		for(int i = 0; i < envelopeList.size(); i++){
    			...
    			if(envelopeList.get(i).getReceiverId().equals(loginId) && (envelopeList.get(i).getReceiveState() == null || "0".equals(envelopeList.get(i).getReceiveState()))){
        			...
        			instationMsgService.updateEnvelope(envelopeList.get(i));
        		}
    		}
    	}
    	return messageView;
    ...
    public ModelAndView showMessage(@RequestParam(value = "messageId", required = true) String messageId, 
    	...
    	if(McSystemConstants.OPER_CODE_EDIT.equals(operType)){
    		// 编辑
    		model.put("operType", operType);
    		return new ModelAndView("mc/instationmessage/newmessage/msg_create", model);
    	}else if(McSystemConstants.OPER_CODE_FORWARD.equals(operType)){
    		// 转发
    		model.put("operType", operType);
    		return new ModelAndView("mc/instationmessage/newmessage/msg_create", model);
    	}else if(McSystemConstants.OPER_CODE_REPLY.equals(operType)){
    		// 回复
    		model.put("operType", operType);
    		return new ModelAndView("mc/instationmessage/newmessage/msg_create", model);
    	}else if(McSystemConstants.OPER_CODE_VIEW.equals(operType)){
    		// 查看
    ...
    public Map<String, Object> ajaxSave(MessageView messageView, @PathVariable("boxType") String boxType) {
    	...
    	if(McSystemConstants.INSTATIONMSG_DRAFT_BOX.equals(boxType)) {
    		//草稿箱保存的话先删除
    		instationMsgService.physicalDelete(envelopeList, messageId);
    	}
    	...
    	//插入
    	boolean flag = instationMsgService.batchInsertEnvelope(envelopeList, message);
        Map<String, Object> model = new HashMap<String, Object>();
    ...
    public Map<String, Object> delete(@PathVariable("messageIds") String messageIds, 
    	...
    	if (messageIds != null) {
    		 ...
    		 for(int i = 0;i < messageIdArray.length;i++){
    			 ...
    			 List<Envelope> envelopeItem = instationMsgService.findEnvelopeListByMessageIdAndLoginId(map);
    			 envelopeList.addAll(envelopeItem);
    		 }
    		 envelopeMap.put("envelopeList", envelopeList);
    		 envelopeMap.put("boxType", boxType);
    		 instationMsgService.delete(envelopeMap);
         }
    	 Map<String, Object> model = new HashMap<String, Object>();
    	 model.put("success", true);
         return model;

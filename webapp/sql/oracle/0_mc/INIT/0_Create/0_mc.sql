...
(
	...
	RECEIVER_ID VARCHAR2(32),	 --  收件人Name
	RECEIVE_STATE VARCHAR(1),    --  收件状态：0：未读；1：已读；2：删除；3：彻底删除；null：未接收； 
	SEND_STATE VARCHAR(1),    --  发送状态：0：未发送；1：已发送；2：删除；3：彻底删除 ；4发送中；5：发送失败
	RESEND_COUNT NUMBER(3),    --  重发次数
...
COMMENT ON COLUMN MC_ENVELOPE_HISTORY.RESEND_COUNT IS '重发次数';
...
COMMENT ON COLUMN MC_ENVELOPE.RESEND_COUNT IS '重发次数';
...
/**参数类型表 */
CREATE TABLE MC_PARAM_TYPE
(
	ID      VARCHAR2(32) NOT NULL,    -- 主键ID 
	MESSAGE_TYPE_CODE VARCHAR2(50),    -- 消息类型表--消息类型编码 
	PARAM_CODE VARCHAR2(50),    -- 参数编码 
	PARAM_NAME VARCHAR2(100),    -- 参数名称 
	SEQ     NUMBER(3),    -- 索引 最大999 
	REMARK  VARCHAR2(200)    -- 备注说明  
);

COMMENT ON TABLE MC_PARAM_TYPE IS '参数类型表';
COMMENT ON COLUMN MC_PARAM_TYPE.ID      IS '主键ID';
COMMENT ON COLUMN MC_PARAM_TYPE.MESSAGE_TYPE_CODE IS '消息类型表--消息类型编码';
COMMENT ON COLUMN MC_PARAM_TYPE.PARAM_CODE IS '参数编码';
COMMENT ON COLUMN MC_PARAM_TYPE.PARAM_NAME IS '参数名称';
COMMENT ON COLUMN MC_PARAM_TYPE.SEQ     IS '索引 最大999';
COMMENT ON COLUMN MC_PARAM_TYPE.REMARK  IS '备注说明 ';

/** 参数值表 */
CREATE TABLE MC_PARAM_VALUE
(
	ID    VARCHAR2(32) NOT NULL,    -- 主键ID 
	PARAM_CODE VARCHAR2(50),    -- 参数类型表--参数编码 
	PARAM_VALUE VARCHAR2(200),    -- 参数值 
	PARAM_AVAILABE CHAR(1),    -- 可用性 0：不可用 1：可用 
	SEQ   NUMBER(3),    -- 索引 最大999 
	REMARK VARCHAR2(200)    -- 备注说明 
);

COMMENT ON TABLE MC_PARAM_VALUE IS '参数值表';
COMMENT ON COLUMN MC_PARAM_VALUE.ID    IS '主键ID';
COMMENT ON COLUMN MC_PARAM_VALUE.PARAM_CODE IS '参数类型表--参数编码';
COMMENT ON COLUMN MC_PARAM_VALUE.PARAM_VALUE IS '参数值';
COMMENT ON COLUMN MC_PARAM_VALUE.PARAM_AVAILABE IS '可用性 0：不可用 1：可用';
COMMENT ON COLUMN MC_PARAM_VALUE.SEQ   IS '索引 最大999';
COMMENT ON COLUMN MC_PARAM_VALUE.REMARK IS '备注说明';

/** 消息类型表 */
CREATE TABLE MC_MESSAGE_TYPE
(
	ID      VARCHAR2(32) NOT NULL,    -- 主键ID 
	MESSAGE_TYPE_CODE VARCHAR2(50) NOT NULL,    -- 消息类型编码 
	MESSAGE_TYPE_NAME VARCHAR2(100),    -- 消息类型名称 
	SQE     NUMBER(5),    -- 索引 最大999 
	REMARK  VARCHAR2(200)    -- 备注说明 
);

COMMENT ON TABLE MC_MESSAGE_TYPE IS '消息类型表';
COMMENT ON COLUMN MC_MESSAGE_TYPE.ID      IS '主键ID';
COMMENT ON COLUMN MC_MESSAGE_TYPE.MESSAGE_TYPE_CODE IS '消息类型编码';
COMMENT ON COLUMN MC_MESSAGE_TYPE.MESSAGE_TYPE_NAME IS '消息类型名称';
COMMENT ON COLUMN MC_MESSAGE_TYPE.SQE     IS '索引 最大999';
COMMENT ON COLUMN MC_MESSAGE_TYPE.REMARK  IS '备注说明';

ALTER TABLE MC_MESSAGE_TYPE ADD CONSTRAINT PK_MC_MESSAGE_TYPE
	PRIMARY KEY (ID);

ALTER TABLE MC_PARAM_VALUE ADD CONSTRAINT PK_MC_PARAM_VALUE 
	PRIMARY KEY (ID);

ALTER TABLE MC_PARAM_TYPE ADD CONSTRAINT PK_MC_PARAM_TYPE 
	PRIMARY KEY (ID);

...
CREATE TABLE MC_ENVELOPE (
  ...
  SENDER_NAME VARCHAR(32) NOT NULL COMMENT '发件人NAME',
  RECEIVER_ID VARCHAR(32)  COMMENT '收件人ID',
  RECEIVER_NAME VARCHAR(32)  COMMENT '收件人NAME',
  RECEIVE_STATE VARCHAR(2)  COMMENT '收件状态：0：未读；1：已读；2：删除；3：彻底删除；NULL：未接收；',
  SEND_STATE VARCHAR(2)  COMMENT '发送状态：0：未发送；1：已发送；2：删除；3：彻底删除；4：发送中；5：发送失败',
  RESEND_COUNT INT COMMENT '重发次数',
...
(
	ID      VARCHAR(32) NOT NULL COMMENT 'ID', 
	MESSAGE_TYPE_CODE VARCHAR(50) COMMENT '消息类型表--消息类型编码',   
	PARAM_CODE VARCHAR(50) COMMENT '参数编码',     
	PARAM_NAME VARCHAR(100) COMMENT '参数名称 ',   
	SEQ     INT COMMENT '索引 最大999',    
...
(
	...
	PARAM_CODE VARCHAR(50) COMMENT '参数类型表--参数编码',  
	PARAM_VALUE VARCHAR(200) COMMENT '参数值',   
	PARAM_AVAILABE CHAR(1) COMMENT '可用性 0：不可用 1：可用',  
	SEQ   INT COMMENT '索引 最大999',   
	REMARK VARCHAR(200) COMMENT '备注说明',    
	PRIMARY KEY  (ID)
) COMMENT='参数值表';
...
(
	ID      VARCHAR(32) NOT NULL COMMENT '主键ID',    
	MESSAGE_TYPE_CODE VARCHAR(50) NOT NULL COMMENT '消息类型编码',  
	MESSAGE_TYPE_NAME VARCHAR(100) COMMENT '消息类型名称',   
	SQE     INT COMMENT '索引 最大999',    
	REMARK  VARCHAR(200) COMMENT '备注说明',   
	PRIMARY KEY  (ID)
) COMMENT='消息类型表';
